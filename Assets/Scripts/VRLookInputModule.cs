...
public class VRLookInputModule : BaseInputModule {
	...
	private PointerEventData GetLookPointerEventData() {
		...
		if (result.distance < interactionDistance)
		{
			lookData.pointerCurrentRaycast = result;
		}
		else
		{
			lookData.pointerCurrentRaycast = new RaycastResult();
		}
